...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def tweedie_z0_given_zt(self, t, zt, node_mask, edge_mask, context):
        gamma_t = self.gamma(t)
        sigma_t = self.sigma(gamma_t, target_tensor=zt)
        ...
        z0 = 1 / alpha_t * zt - (sigma_t / alpha_t) * eps_t
        return z0
    
    def forward_to_zs_given_z0(self, s, z0, node_mask):
        """Samples from zs ~ p(zs | zt). Only used during sampling."""
        gamma_s = self.gamma(s)
        sigma_s = self.sigma(gamma_s, target_tensor=z0)
        alpha_s = self.alpha(gamma_s, target_tensor=z0)

        # true eps
        eps = self.sample_combined_position_feature_noise(
            n_samples=z0.size(0), n_nodes=z0.size(1), node_mask=node_mask)
 
        zs = alpha_s * z0 + sigma_s * eps
        zs = zs * node_mask
    ...
    @torch.no_grad()
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, context, args, fix_noise=False, time_classifier=None):
        tweedie_ub = args.tweedie_ub
        mc_sigma = args.mc_sigma
        mc_num = args.mc_num
        
        if args.mc_time:
            node_mask_exp = node_mask.squeeze(2).repeat(mc_num, 1)
            edge_mask_exp = node_mask_exp.unsqueeze(1) * node_mask_exp.unsqueeze(2)
            diag_mask = ~torch.eye(edge_mask_exp.size(1), dtype=torch.bool).unsqueeze(0)
            diag_mask = diag_mask.to(node_mask.device)
            edge_mask_exp *= diag_mask
            edge_mask_exp = edge_mask_exp.view(mc_num * n_samples * n_nodes * n_nodes, 1)
            node_mask_exp = node_mask_exp.unsqueeze(2)
            context_exp = context.repeat(mc_num, 1, 1)
        ...
        pbar = tqdm(reversed(range(0, self.T)), desc='Sampling', leave=True)
        for s in pbar:
            if s < tweedie_ub:
                t_array = t_pred_array
                # s_array = torch.where(t_array > 0, t_array - 1 / self.T, torch.tensor(0.0))
            ...
            # Original Diffusion
            if s >= tweedie_ub or tweedie_ub == -1:
                z = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, context,
                                                fix_noise=fix_noise)

            # Tweedie Method #TODO: time gap in tweedie method?
            elif s < tweedie_ub:
                z0 = self.tweedie_z0_given_zt(t_array, z, node_mask, edge_mask, context)
                z = self.forward_to_zs_given_z0(s_array, z0, node_mask)
            ...
            if s <= tweedie_ub:
                if args.mc_time:
                    # mc sampling
                    z_mc = torch.distributions.Normal(z, mc_sigma).sample((mc_num,))
                    z_mc = z_mc.view(-1, n_nodes, self.n_dims + self.in_node_nf)
                    # time prediction
                    pred_t = time_classifier.t_pred(z_mc, node_mask_exp, edge_mask_exp, context_exp)
                    pred_t = torch.argmax(pred_t, dim=1).float()
                    pred_t = pred_t.view(mc_num, -1)
                    pred_t = torch.mean(pred_t, dim=0)   
                else:
                    pred_t = time_classifier.t_pred(z, node_mask, edge_mask, context)
                    pred_t = torch.argmax(pred_t, dim=1).float()
                    
                if args.time_window: 
                    clipped_time_gap = torch.clamp(pred_t - s, min=args.window_min, max=args.window_max) # prev -40,20
                    t_pred_array = (clipped_time_gap + s).unsqueeze(-1) / self.T  
                else:
                    t_pred_array = pred_t.unsqueeze(-1) / self.T

                wandb.log({'method time gap': (pred_t - s).mean().item()})    
                    
            pbar.set_description(f'Sampling denoising step={s}')
