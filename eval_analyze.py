...
from time_regressor import data_utils
...
def analyze_and_save(args, eval_args, device, generative_model,
                     ...
                     batch_size=10, save_to_xyz=False, time_classifier=None):
    
    for attr in dir(eval_args):
        if not attr.startswith('__') and not callable(getattr(eval_args, attr)) and not hasattr(args, attr):
            setattr(args, attr, getattr(eval_args, attr))
    ...
    for i in range(int(n_samples/batch_size)):
        ...
        one_hot, charges, x, node_mask = sample(
            args, device, generative_model, dataset_info, prop_dist=prop_dist, nodesxsample=nodesxsample, time_classifier=time_classifier)
...
def get_time_classifier(dir_path='', device='cpu', dataset_info=None, dataloader_train=None):
    with open(join(dir_path, 'args.pickle'), 'rb') as f:
        args_time_classifier = pickle.load(f)
    args_time_classifier.device = device
    time_classifier, _, _ = data_utils.get_model(args_time_classifier, device, dataset_info, dataloader_train)
    time_classifier_state_dict = torch.load(join(dir_path, 'generative_model_ema.npy'), #TODO: ema model?
                                            map_location=torch.device('cpu'))
    time_classifier.load_state_dict(time_classifier_state_dict)
    return time_classifier
...
def main():
    ...
    parser.add_argument('--project_name', type=str, default='Final Method')
    parser.add_argument('--time_classifiers_path', type=str, default='time_regressor/outputs/time_regressor_4')
    parser.add_argument('--property', type=str, default='alpha',
                    help="'alpha', 'homo', 'lumo', 'gap', 'mu', 'Cv'")
    parser.add_argument('--zeroth_coef', type=float, default=1.0)
    parser.add_argument('--tweedie_ub', type=int, default=0)
    parser.add_argument('--online_ub', type=int, default=0)
    parser.add_argument('--online_lb', type=int, default=0)
    parser.add_argument('--mc_online', action='store_true', default=False, help="Enable MC online")
    parser.add_argument('--mc_time', action='store_true', default=False, help="Enable MC time")
    parser.add_argument('--mc_sigma', type=float, default=0.3, help="Set MC sigma value")
    parser.add_argument('--mc_num', type=int, default=200, help="Set MC number")
    parser.add_argument('--clip', type=float, default=1.0)
    parser.add_argument('--distract', action='store_true', default=False, help="Enable distraction")
    parser.add_argument('--time_correct', action='store_true', default=False, help="Enable time correction for online guidance")
    parser.add_argument('--time_window', action='store_true', default=False)
    parser.add_argument('--window_min', type=int, default=-10)
    parser.add_argument('--window_max', type=int, default=10)
    parser.add_argument('--wandb_usr', type=str)
    parser.add_argument('--exp_name', type=str, default='eegsde_geom_fingerprint')

    eval_args, unparsed_args = parser.parse_known_args()

    assert eval_args.model_path is not None
    
    eval_args.wandb_usr = utils.get_wandb_username(eval_args.wandb_usr)
    # Wandb config
    if eval_args.no_wandb:
        mode = 'disabled'
    else:
        mode = 'online' if eval_args.online else 'offline'
    kwargs = {'entity': eval_args.wandb_usr, 'name': eval_args.exp_name, 'project': eval_args.project_name, 'config': eval_args,
    ...
    time_classifier = get_time_classifier(eval_args.time_classifiers_path, 
                                          args.device, dataset_info, dataloaders['train']).to(args.device)
    ...
    stability_dict, rdkit_metrics = analyze_and_save(
        ...
        batch_size=eval_args.batch_size_gen, save_to_xyz=eval_args.save_to_xyz, time_classifier=time_classifier)
    ...
    wandb.log({'Avg Validity': rdkit_metrics[0], 'Uniqueness': rdkit_metrics[1], 'Novelty': rdkit_metrics[2]})
